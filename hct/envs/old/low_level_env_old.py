...
from hct.training.configs import NetworkArchitecture, SMALL_TRANSFORMER_CONFIGS
...
class LowLevelEnv(PipelineEnv):
  ...
  def __init__(
    self,
    morphology: Literal['ant', 'humanoid'] = 'ant_original',
    ...
    architecture_configs = SMALL_TRANSFORMER_CONFIGS,
    **kwargs
  ):
    ...
    num_attn_heads = self.network_architecture.configs['num_heads'] if self.network_architecture.name=='Transformer' else 2
  ...
  def step(self, state: State, action: jp.ndarray) -> State:
    ...
    state.metrics.update(
        ...
        distance_from_origin=safe_norm(pipeline_state.x.pos[0]),
  ...
  def _get_intrinsic_reward(self, previous_state: State, current_state: State, goal: Goal) -> float:
    def difference_reward():
      prev_goal_distance= self._goal_dist(previous_state, goal, frame = 'world', rot_dist=self.rot_dist)
      current_goal_distance = self._goal_dist(current_state, goal, frame = 'world', rot_dist=self.rot_dist)
  ...
  def _root_goal_dist(self, state: base.State, goal: Goal, rot_dist=True):
  ...
  def _goal_dist(self, state: base.State, goal: Goal, frame: Literal['world', 'relative'], rot_dist=True):
    """
