...
class FlatHurdlesGapsEnv(PipelineEnv):
  ...
  def __init__(
    ...
    unhealthy_cost=-1500.0, 
    reward_milestone = 100,
    reward_type: Literal['sparse', 'dense'] = 'sparse',
    reward_movement: Literal['position', 'velocity'] = 'position',
  ...
  def _reward(self, obstacles_complete, state: base.State):
    ...
    next_obstacle_coord = self.obstacle_completion_coords[obstacles_complete.astype(int)]
    passed_next_obstacle = state.x.pos[0, 1] >= next_obstacle_coord

    def sparse_reward():
      return self.reward_milestone * passed_next_obstacle - (1.0 - passed_next_obstacle)

    def dense_reward():
      if self.reward_movement == 'position':
        return state.x.pos[0, 1]
      else:
