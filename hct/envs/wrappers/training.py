...
class EvalWrapper(Wrapper):
  ...
  def step(self, state: State, action: jp.ndarray) -> State:
    ...
    reward_metrics = {key: value for key, value in nstate.metrics.items() if 'reward' in key}
    other_metrics = {key: value for key, value in nstate.metrics.items() if 'reward' not in key}
    episode_reward_metrics = {key: value for key, value in state_metrics.episode_metrics.items() if 'reward' in key}
    episode_other_metrics = {key: value for key, value in state_metrics.episode_metrics.items() if 'reward' not in key}
      
    episode_reward_metrics = jax.tree_util.tree_map(
        lambda a, b: a + b * state_metrics.active_episodes,
        episode_reward_metrics,
        reward_metrics,
    )
    episode_metrics = jax.tree_util.tree_map(
        lambda a, b: a + (b-a) * state_metrics.active_episodes,
        episode_other_metrics,
        other_metrics,
    )

    episode_metrics.update(episode_reward_metrics)
