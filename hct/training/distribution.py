...
class ParametricDistribution(abc.ABC):
  ...
  def __init__(self, 
    ...
    self._sum_axes = (-1, -2) if network_architecture=='Transformer' else (-1)
  ...
  def log_prob(self, parameters, actions):
    ...
    if self._event_ndims == 1:
      log_probs = jnp.sum(log_probs, axis=self._sum_axes)  # sum over action dimension
    return log_probs
  ...
  def entropy(self, parameters, seed):
    ...
    if self._event_ndims == 1:
      entropy = jnp.sum(entropy, axis=self._sum_axes)
    return entropy
