...
class MidLevelEnv(PipelineEnv):
  ...
  def __init__(
    ...
    self.horizon = self.low_level_env.horizon * 1.75
  ...
  def take_action(self, pipeline_state, action, rng):
    # Take action
    low_level_goal = self.get_low_level_goal(action, pipeline_state)
  ...
  def get_obs(self, state: base.State, goal: Goal, goal_dist = True) -> jp.ndarray:
    ...
    sx_, sxd_ = world_to_relative(state, self.low_level_env)
    ...
    if hasattr(self, 'state_below'):
      if self.state_below:
        s_obs = jp.concatenate([concatenate_attrs(_mask_root(sx_)), concatenate_attrs(sxd_)], axis=-1)
        g_obs = jp.concatenate([concatenate_attrs(gx), concatenate_attrs(gxd), importance], axis=-1)
        obs = jp.concatenate([s_obs.flatten(), g_obs.flatten()])
      else:
