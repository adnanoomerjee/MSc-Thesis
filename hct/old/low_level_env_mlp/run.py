...
LOW_LEVEL_ENV_PARAMETER2 = {
    'position_goals': [True],
    'velocity_goals': [None],
    'distance_reward': ['relative'],
    'goal_importance': [True],
    'architecture_configs': [VLARGE_MLP_CONFIGS, VVLARGE_MLP_CONFIGS]
    }

LOW_LEVEL_ENV_PARAMETER3 = {
    'position_goals': [True],
    'velocity_goals': [None],
    'distance_reward': ['relative'],
    'goal_importance': [True],
    'architecture_configs': [MAX_MLP_CONFIGS]
    }
...
def hyperparameter_sweep():
    ...
    other_params = LOW_LEVEL_ENV_PARAMETER2
    other_param_names = other_params.keys()
    other_param_values = other_params.values()
    other_param_combinations = list(itertools.product(*other_param_values))
    env_parameters += [dict(zip(other_param_names, combination)) for combination in other_param_combinations]

    other_params = LOW_LEVEL_ENV_PARAMETER3
    other_param_names = other_params.keys()
    other_param_values = other_params.values()
    other_param_combinations = list(itertools.product(*other_param_values))
    env_parameters += [dict(zip(other_param_names, combination)) for combination in other_param_combinations]
